 ...
 * @return array
 */
function get_calculation_result($data_arr,$room,$room_number) {

    switch ($room['cornice_article']) {
        case '1.50.501':
            return get_first_method($data_arr,$room,$room_number);
            break;
        /*case '1.50.504':
        ...
        case '1.50.504':
        case '1.50.524':
            return get_third_method($data_arr,$room,$room_number);
            break;
    }
}
 ...
 * @return array
 */
function get_first_method($data_arr,$room,$room_number) {
    ...
    foreach($walls as $k => $wall) {
                ...
                if(!empty($r) && count($r) <= 2) {
                    ...
                    if($needed_piece != '') {
                        $arr = array(
                            ...
                            'room_number' => $rest[$needed_piece]['room_number'],
        ...
        for($n = 0; $n < $total_cornice_qty; $n++) {
            ...
            if($cutting_item == 0) $cutting_item = $total_need_cutting;
            if($cutting_item != 0) {
            ...
            if($total_need_cutting != 0) {
                ...
                } elseif ($cutting_type == 'trimming' && $wall['corner_1']['type'] == 'trimming' && $wall['corner_1']['trimming_fit'] != 'yes') {
                ...
                if($total_cornice_qty > 3) { //пилим с последнего целого
                    if($n == $total_cornice_qty - 1) {
                        if($rapport_rest > 1) {
                            ...
                            $rest_item['room_number'] = $room_number;
                            $rest_item['wall'] = $k + 1;
 ...
 * @return array
 */
function get_third_method($data_arr,$room,$room_number) {
