...
foreach($subdomains AS $key => $subdomain) {
	...
	//if ($key <> 3182) continue;

	$DEALER_INFO = get_dealer_info($key);
	$DEALER_ID = $DEALER_INFO['ID']; 
	$DEALER_NAME = dealer_name_clear($DEALER_INFO['NAME']);
	$EMAIL = $DEALER_INFO['DEALER_EMAIL'];
	$LOC_NAME = $DEALER_INFO['LOC_NAME'];

	$DEALER_INFO2 = get_dealer_info($key, true);
	$EMAIL2 = $DEALER_INFO2['DEALER_EMAIL'];
...
/* --- Очистка названия дилера --- */

function dealer_name_clear($name) {
	$name = (string)$name;
