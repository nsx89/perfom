...
if (!function_exists('yandexPrepareItems'))
{
	function yandexPrepareItems(array &$list, array $options)
	{
		foreach (array_keys($list) as $index)
		{
			...
			if ($row['FLEX']['VALUE'] == 'Y') {
 				$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/60/" . $row['ARTICUL']['VALUE'] . ".60.png";
			$img_path_web = "/cron/catalog/data/images/60/" . $row['ARTICUL']['VALUE'] . ".60.png";
 				if (!file_exists($img_path)) {
     				$img_path_web = "/cron/catalog/data/images/nope.jpg";
 				}
				} else {
 					$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/10/" . $row['ARTICUL']['VALUE'] . ".10.png";
				$img_path_web = "/cron/catalog/data/images/10/" . $row['ARTICUL']['VALUE'] . ".10.png";
             			if (!file_exists($img_path)) {
                 				$img_path_web = "/cron/catalog/data/images/nope.jpg";
 					}
			}

			$strFile = $usedProtocol.$arAcc['SERVER_NAME'].CHTTP::urnEncode($img_path_web, 'utf-8');

			$picturePath .= "<picture>".$strFile."</picture>\n";
...
if ($firstStep)
{
	...
	if (empty($arRunErrors))
	{
		...
		fwrite($fp, '<url>https://'.htmlspecialcharsbx($ar_iblock['SERVER_NAME'])."</url>\n");
		...
		if (!empty($items))
		{
			foreach ($itemIdsList as $id)
			{
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					$dFilter = Array('IBLOCK_ID' => $row['IBLOCK_ID'], 'ACTIVE' => 'Y', 'ID' => $row['ID']);
               $ddb_list = CIBlockElement::GetList(Array("SORT" => "ASC"), $dFilter);
					...
					$itemsContent .= "<url>https://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx(__get_product_link($dItem))."</url>\n";
					...
					$strFile = "https://".$ar_iblock['SERVER_NAME'].CHTTP::urnEncode($img_path_web, 'utf-8');
					

					$itemsContent.="<picture>".$strFile."</picture>\n";
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							case 'name':
								...
								$itemsContent .= "<name>".yandex_text2xml(__get_product_name($dItem))."</name>\n";
								break;
							case 'description':
