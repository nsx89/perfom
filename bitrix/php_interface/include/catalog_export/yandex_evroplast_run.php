	...
	if (is_array($YANDEX_EXPORT))
	{
		...
		foreach ($YANDEX_EXPORT as $ykey => $yvalue)
		{
			...
			while ($arAcc = $db_acc->Fetch())
			{
				...
				$strTmpCat.= '<category url="https://evroplast.ru/'.$arAcc['CODE'].'" id="'.$arAcc['ID'].'"'.($arAcc['IBLOCK_SECTION_ID'] > 0 ? ' parentId="'.$arAcc['IBLOCK_SECTION_ID'].'"' : '').'>'.yandex_text2xml($arAcc['NAME'], true).'</category>'."\n";
