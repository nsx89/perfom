...
if (strlen($strExportErrorMessage)<=0)
{
	...
	fwrite($fp, "<shop>\n");
	fwrite($fp, "<name>".$APPLICATION->ConvertCharset('ЕВРОПЛАСТ' , LANG_CHARSET, 'windows-1251')."</name>\n");
	fwrite($fp, "<company>".$APPLICATION->ConvertCharset('ООО "Декор"', LANG_CHARSET, 'windows-1251')."</company>\n");
	fwrite($fp, "<url>https://".htmlspecialcharsbx(strlen($SETUP_SERVER_NAME) > 0 ? $SETUP_SERVER_NAME : COption::GetOptionString("main", "server_name", ""))."</url>\n");
	...
	*/
	...
	// массив композит в игнор
	$composite = array(1614,1615,1616,1617);
	//*****************************************//
	...
	if (is_array($YANDEX_EXPORT))
	{
		...
		foreach ($YANDEX_EXPORT as $ykey => $yvalue)
		{
			...
			$db_acc = CIBlockSection::GetList(array("LEFT_MARGIN" => "ASC"), $filter, false, array('ID', 'IBLOCK_SECTION_ID', 'NAME', 'CODE'));
			...
			while ($arAcc = $db_acc->Fetch())
			{
				...
				// Игнорируем композит
				if (in_array($arAcc['ID'],$composite)) continue; // пропуск групп композитов

                $filterCat = array("IBLOCK_ID" => $yvalue, "SECTION_ID" => $arAcc['ID'], "INCLUDE_SUBSECTIONS" => 'Y',  "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", 'PROPERTY_SELLOUT' => 'Y');
                $resCat = CIBlockElement::GetList(array(), $filterCat, false, false, $arSelect);
				
                if(intval($resCat->SelectedRowsCount())>0) {
            ...
            $filter = array("IBLOCK_ID" => $yvalue, "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", 'PROPERTY_SELLOUT' => 'Y');
			...
			{
				// Игнорируем композит
				if (in_array($arAcc['IBLOCK_SECTION_ID'],$composite)) continue; // пропуск групп композитов
				...
				$str_QUANTITY = (float)$arAcc['CATALOG_QUANTITY'];
				...
				*/
			$str_AVAILABLE = ' available="true"'; // наличие Dem
				...
				*/
				$dFilter = Array('IBLOCK_ID' => $yvalue, 'ACTIVE' => 'Y', 'ID' => $arAcc['ID']);
                		$ddb_list = CIBlockElement::GetList(Array("SORT" => "ASC"), $dFilter);
				$dItem = $ddb_list->GetNextElement();
				$dItem = array_merge($dItem->GetFields(), $dItem->GetProperties());
				
				$minPriceCurrency = $RUR;		// Dem - валюта
				$minPrice = __get_product_cost($dItem); // Dem - стоимость
				$oldPrice = _makeprice($dItem['OLD_PRICE']['VALUE']);
				...
				/*$strTmpOff.= "<url>https://".$arAcc['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
				*/

				$strTmpOff.= "<url>https://".$arAcc['SERVER_NAME'].htmlspecialcharsbx(__get_product_link($dItem))."</url>\n"; // Dem
				...
				if (intval($arAcc["DETAIL_PICTURE"])>0 || intval($arAcc["PREVIEW_PICTURE"])>0)
				{
					...
					if (is_array($arPictInfo))
					{
						if(substr($arPictInfo["SRC"], 0, 1) == "/")
							$strFile = "https://".$arAcc['SERVER_NAME'].CHTTP::urnEncode($arPictInfo["SRC"], 'utf-8');
						else
				...
				*/
				if ($dItem['FLEX']['VALUE'] == 'Y') {
					$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/600/" . $dItem['ARTICUL']['VALUE'] . ".600.png";
					$img_path_web = "/cron/catalog/data/images/600/" . $dItem['ARTICUL']['VALUE'] . ".600.png";
					if (!file_exists($img_path)) { // старый контент
						$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/60/" . $dItem['ARTICUL']['VALUE'] . ".60.png";
						$img_path_web = "/cron/catalog/data/images/60/" . $dItem['ARTICUL']['VALUE'] . ".60.png";
						if (!file_exists($img_path)) {
							$img_path_web = "/cron/catalog/data/images/nope.jpg";
					}}} else {
						$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/100/" . $dItem['ARTICUL']['VALUE'] . ".100.png";
						$img_path_web = "/cron/catalog/data/images/100/" . $dItem['ARTICUL']['VALUE'] . ".100.png";
						if (!file_exists($img_path)) { // старый контент
    					$img_path = $_SERVER["DOCUMENT_ROOT"]."/cron/catalog/data/images/10/" . $dItem['ARTICUL']['VALUE'] . ".10.png";
						$img_path_web = "/cron/catalog/data/images/10/" . $dItem['ARTICUL']['VALUE'] . ".10.png";
                			if (!file_exists($img_path)) {
                    				$img_path_web = "/cron/catalog/data/images/nope.jpg";
    					}}
					}
				...
				$strFile = "https://".$arAcc['SERVER_NAME'].CHTTP::urnEncode($img_path_web, 'utf-8');
				

				$strTmpOff.="<picture>".$strFile."</picture>\n";

				//$strTmpOff.= "<name>".yandex_text2xml($arAcc["~NAME"], true)."</name>\n";
				if (is_array($dItem['COMPOSITEPART']['VALUE']) && count($dItem['COMPOSITEPART']['VALUE'])) { // Dem
					$strTmpOff.= "<name>".yandex_text2xml($dItem['NAME'], true)."</name>\n";
				} else {
					//$strTmpOff.= "<name>".yandex_text2xml($dItem['NAME'].' '.$dItem['ARTICUL']['VALUE'], true)."</name>\n";
					$strTmpOff.= "<name>".yandex_text2xml(__get_product_name($dItem), true)."</name>\n";
				}
