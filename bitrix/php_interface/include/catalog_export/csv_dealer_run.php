<?
//<title>CSV Export Dealer (Dem)</title>
...
if (empty($arRunErrors))
{
	...
	if (empty($arRunErrors))
	{
		...
		$dbIBlockElement = CIBlockElement::GetList(
				array('ID' => 'ASC'),
				array("IBLOCK_ID" => $IBLOCK_ID, 'CHECK_PERMISSIONS' => 'N', 'ACTIVE' => 'Y'),
				false,
				false,
				$selectArray
			);
		while ($obIBlockElement = $dbIBlockElement->GetNextElement())
		{
			...
			$obItem = array_merge($obIBlockElement->GetFields(), $obIBlockElement->GetProperties());
			if (is_array($obItem['COMPOSITEPART']['VALUE'])) continue;
				...
				foreach ($arResPrices as $arPrice)
				{
					...
					foreach($arNeedFields as $field_name)
					{
						if (strncmp($field_name, "IE_", 3) == 0)
							if ($field_name == "IE_NAME") {
							// замена на полное название

							$arTuple[] = mb_convert_encoding(__get_product_name($obItem), "UTF-8", "auto");
							} else {
