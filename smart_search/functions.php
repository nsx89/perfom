...
function createProductCache($count_id,$return = false) {
    ...
    $cacheTtl = 60*60*24 * 30; // на месяц - срок годности кеша (в секундах)
    ...
    elseif ($cache->startDataCache())
    {
        ...
        $products = createProductList($count_id, $return);
...
function createProductList($count_id, $return = false) {
    ...
    //список разделов
    $section_list = Array();
    $sec_res = CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => IB_CATALOGUE), false, Array());
    ...
    if (!empty($FIND_BY_ARTICUL_VALUE)) {
        $ARTS_OFF = 
        $resSS = CIBlockElement::GetList(Array('NAME'=>'asc','PROPERTY_ARTICUL'=>'asc'), Array('IBLOCK_ID'=>IB_CATALOGUE, 'PROPERTY_ARTICUL' => $FIND_BY_ARTICUL_VALUE, '!TAGS' => 'OFF'), false, array('nPageSize' => 10), array());
        $FIND_BY_ARTICUL = true;
    }
    else {
        $resSS = CIBlockElement::GetList(Array('IBLOCK_ID'=>IB_CATALOGUE, 'ID' => '999999999'), $arFilterSS, false, array(), array());
    }
    /* --- // --- */
